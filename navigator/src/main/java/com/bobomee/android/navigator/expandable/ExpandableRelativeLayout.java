...
import com.bobomee.android.navigator.expandable.interfaces.ExpandableChange;
...
public class ExpandableRelativeLayout extends RelativeLayout implements ExpandableLayout {
  ...
  private void init(final Context context, final AttributeSet attrs, final int defStyleAttr) {
    final TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.expandableLayout, defStyleAttr, 0);
    ...
    defaultChildIndex =
        a.getInteger(R.styleable.expandableLayout_ael_defaultChildIndex, Integer.MAX_VALUE);
    defaultPosition = a.getDimensionPixelSize(R.styleable.expandableLayout_ael_defaultPosition,
        Integer.MIN_VALUE);
    final int interpolatorType =
        a.getInteger(R.styleable.expandableLayout_ael_interpolator, Utils.LINEAR_INTERPOLATOR);
    a.recycle();
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    for (int i = 0; i < getChildCount(); i++) {
      ...
      childSizeList.add(
          isVertical() ? view.getMeasuredHeight() + params.topMargin + params.bottomMargin
   ...
   * {@inheritDoc}
   */
  @Override public void toggle(final long duration, final @Nullable TimeInterpolator interpolator) {
   ...
   * {@inheritDoc}
   */
  @Override public void expand(final long duration, final @Nullable TimeInterpolator interpolator) {
    if (isAnimating) return;
   ...
   * {@inheritDoc}
   */
  @Override public void collapse(final long duration,
      final @Nullable TimeInterpolator interpolator) {
    if (isAnimating) return;
  ...
  @Override public void setExpanded(boolean expanded) {
    ...
    if ((expanded && (currentPosition == layoutSize)) || (!expanded
   ...
   * {@inheritDoc}
   */
  @Override public void setInterpolator(@NonNull final TimeInterpolator interpolator) {
  ...
  public void moveChild(int index, long duration, @Nullable TimeInterpolator interpolator) {
    ...
    createExpandAnimator(getCurrentPosition(), destination, duration,
        interpolator == null ? this.interpolator : interpolator).start();
  }
   ...
   */
  private ValueAnimator createExpandAnimator(final int from, final int to, final long duration,
